/* intentionally unchanged structure; recovery link added by markup below */
"use client";
...
export default function LoginPage() {
  ...
  async function submit(event:FormEvent<HTMLFormElement>) {
    ...
    if(mode==="login"){const {error}=await supabase.auth.signInWithPassword({email,password});if(error)setMessage(error.message);else router.push("/");}
    else{const {data,error}=await supabase.auth.signUp({email,password,options:{data:{username:username.trim().toLowerCase(),display_name:username.trim()}}});if(error||!data.user)setMessage(error?.message??"Vertex could not create the account.");else{setMessage("Account created. Check your email if verification is enabled.");setMode("login");}}
    setBusy(false);
  }

  return <main className="page" style={{maxWidth:700}}><div className="card pad" style={{padding:28}}>
    ...
    <form className="form" style={{marginTop:22}} onSubmit={submit}>
      ...
      {mode==="login"&&<Link className="meta" href="/forgot-password" style={{textAlign:"center"}}>Forgot your password?</Link>}
    </form>
