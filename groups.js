...
async function membersView(d){
  return '<div class="panel"><div class="section"><div><div class="eyebrow">Community</div><h3>Members</h3></div><span class="chip">'+fmt(d.g.member_count)+' members</span></div><div class="group-member-list">'+d.members.map(m=>{const p=d.profileBy.get(m.user_id),manage=d.mine&&(d.mine.role==='owner'||d.mine.role==='admin'||d.mine.role==='moderator')&&m.user_id!==d.u?.id&&m.role!=='owner';return '<div class="group-member-row">'+profile(p)+'<span class="group-role">'+esc(m.banned?'banned':m.role)+'</span>'+(manage?'<div class="member-actions">'+(m.banned?'<button class="btn sm" data-unban="'+m.user_id+'">Unban</button>':'<button class="btn sm" data-remove-member="'+m.user_id+'">Remove</button><button class="btn sm danger" data-ban-member="'+m.user_id+'">Ban</button>')+(d.mine.role==='owner'?'<select class="select member-role" data-role-user="'+m.user_id+'"><option '+(m.role==='member'?'selected':'')+' value="member">Member</option><option '+(m.role==='moderator'?'selected':'')+' value="moderator">Moderator</option><option '+(m.role==='admin'?'selected':'')+' value="admin">Admin</option></select>':'')+'</div>':'')+'</div>'}).join("")+'</div></div>';
}
...
function bindInner(d){
  ...
  document.querySelectorAll("[data-role-user]").forEach(s=>s.onchange=async()=>await action("vertex_set_group_role",{p_group:currentId(),p_target:s.dataset.roleUser,p_role:s.value},"Role updated."));
