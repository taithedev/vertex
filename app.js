...
function auth(){return '<div class="modalbg" id="mb"><div class="modal"><div class="modalhead"><div><span class="age-badge">12+</span><h2 id="at">Welcome to Vertex</h2><p id="as" style="color:var(--muted)">Sign in to continue.</p></div><button class="close" id="cx">×</button></div><div class="form" style="margin-top:17px"><button class="btn auth-google" id="google-auth">Continue with Google</button><div class="form-divider"><span>or</span></div><input class="input" id="ae" type="email" placeholder="Email"><input class="input" id="ap" type="password" placeholder="Password"><input class="input" id="ap2" type="password" placeholder="Confirm password" style="display:none"><input class="input" id="au" placeholder="Username" style="display:none"><label id="dob-wrap" style="display:none">Date of birth<input class="input" id="adob" type="date"></label><label id="terms-wrap" style="display:none"><input id="agree" type="checkbox"> I agree to the Vertex Terms and Community Guidelines.</label><button class="btn primary" id="goauth">Sign in</button><button class="btn" id="toggle">Create account</button><button class="btn" id="forgot">Forgot password?</button></div></div></div>'}
function openAuth(){
 document.querySelector("#modal").innerHTML=auth();
 let signup=false;
 const sync=()=>{
   $("#au").style.display=signup?"block":"none";
   $("#ap2").style.display=signup?"block":"none";
   $("#dob-wrap").style.display=signup?"block":"none";
   $("#terms-wrap").style.display=signup?"block":"none";
   $("#at").textContent=signup?"Create your Vertex account":"Welcome to Vertex";
   $("#as").textContent=signup?"Vertex is 12+. Your date of birth is used for age eligibility.":"Sign in to continue.";
   $("#goauth").textContent=signup?"Create account":"Sign in";
   $("#toggle").textContent=signup?"Back to sign in":"Create account";
 };
 sync();
 $("#cx").onclick=()=>$("#modal").innerHTML="";
 $("#toggle").onclick=()=>{signup=!signup;sync()};
 $("#google-auth").onclick=async()=>{
   const x=await db.auth.signInWithOAuth({provider:"google",options:{redirectTo:location.origin+location.pathname+"#home"}});
   if(x.error)toast(x.error.message,false);
 };
 $("#forgot").onclick=async()=>{
   const e=$("#ae").value.trim();
   if(!e)return toast("Enter your email first",false);
   const x=await db.auth.resetPasswordForEmail(e,{redirectTo:location.origin+location.pathname+"#reset-password"});
   if(x.error)return toast(x.error.message,false);
   toast("If the account exists, a reset email was sent.");
 };
 $("#goauth").onclick=async()=>{
   const e=$("#ae").value.trim(),p=$("#ap").value,p2=$("#ap2").value;
   if(!e||!p)return toast("Email and password are required",false);
   if(signup&&p!==p2)return toast("Passwords do not match.",false);
   let x;
   if(signup){
     const dob=$("#adob").value,u=$("#au").value.trim().toLowerCase();
     if(!dob||!is12(dob))return toast("Vertex requires users to be at least 12.",false);
     if(!/^[a-z0-9_]{3,24}$/.test(u))return toast("Username must be 3–24 lowercase characters.",false);
     if(!$("#agree").checked)return toast("Please accept the Terms and Community Guidelines.",false);
     x=await db.auth.signUp({email:e,password:p});
     if(x.error)return toast(x.error.message,false);
     if(x.data.user){
       const pr=await db.from("vertex_profiles").upsert({user_id:x.data.user.id,username:u,display_name:u,birth_date:dob,safety_acknowledged_at:new Date().toISOString()},{onConflict:"user_id"});
       if(pr.error)return toast(pr.error.message,false);
       await db.from("vertex_currency_accounts").upsert({user_id:x.data.user.id,balance:100},{onConflict:"user_id"});
     }
   }else{
     x=await db.auth.signInWithPassword({email:e,password:p});
     if(x.error)return toast(x.error.message,false);
   }
   $("#modal").innerHTML="";
   await load();
