...
window.__vertexDb=db;window.vertexToast=toast;
...
async function load(){
 ...
 if(S.u){
   ...
   if(window.VertexTheme) window.VertexTheme.apply(S.p?.theme||localStorage.getItem("vertex-theme")||"dark",S.p?.accent_color||localStorage.getItem("vertex-accent")||"#a98cff");
 }
...
function settings(){
 if(!S.u)return '<div class="panel empty"><div><h2>Sign in to open Settings</h2><p>Manage your Vertex account, appearance, security and support from one place.</p><button class="btn primary" id="settings-login">Sign in</button></div></div>';
 const theme=S.p?.theme||"dark",accent=S.p?.accent_color||"#a98cff";
 return '<div class="section"><div><div class="eyebrow">Account</div><h2>Settings</h2><p class="page-sub">Appearance, profile, privacy, notifications and security.</p></div></div>'+
 '<div class="settings-grid">'+
 '<div class="panel"><div class="eyebrow">Appearance</div><h3>Theme</h3><p class="page-sub">Switch instantly. Your choice is saved to this account and this browser.</p><div class="form"><label>Theme<select class="select" id="theme-choice"><option value="dark" '+(theme==="dark"?"selected":"")+'>Dark</option><option value="light" '+(theme==="light"?"selected":"")+' >Light</option><option value="system" '+(theme==="system"?"selected":"")+'>System</option></select></label><label>Accent color<div class="accent-row"><input id="accent-choice" type="color" value="'+esc(accent)+'"><span id="accent-value">'+esc(accent)+'</span></div></label></div></div>'+
 '<div class="panel"><div class="eyebrow">Vertex Coins</div><h3>'+fmt(S.currency)+' Coins</h3><p class="page-sub">Group creation costs exactly 100 Coins. Currency changes are handled server-side.</p><button class="btn primary" id="daily-reward">Claim 25 coins</button><button class="btn" data-r="groups">Browse groups</button></div>'+
 '</div>'+
 '<div class="cols settings-section">'+
 '<div class="panel"><div class="eyebrow">Profile</div><h3>Your identity</h3><div class="form"><label>Display name<input class="input" id="dn" maxlength="40" value="'+esc(S.p?.display_name||"")+'"></label><label>Username<input class="input" id="un" maxlength="24" value="'+esc(S.p?.username||"")+'"></label><label>Bio<textarea class="area" id="bio" maxlength="280">'+esc(S.p?.bio||"")+'</textarea></label><label>Date of birth<input class="input" id="dob" type="date" value="'+esc(S.p?.birth_date||"")+'"></label><button class="btn primary" id="save">Save profile</button><a class="btn" href="./account.html" style="text-decoration:none">Open full profile</a></div></div>'+
 '<div class="panel"><div class="eyebrow">Privacy & security</div><h3>Account controls</h3><p class="page-sub">'+esc(S.u.email||"")+'</p><div class="form"><button class="btn" id="reset-password">Change password</button><button class="btn danger" id="logout">Sign out</button><a class="btn" href="./account.html" style="text-decoration:none">Profile & avatar</a></div></div>'+
 '</div>'+
 '<div class="panel support-shell settings-section"><div class="support-head"><div><div class="eyebrow">Support</div><div class="support-title">Vertex Support</div><div class="support-sub">Get help with accounts, games, groups, coins, settings and other Vertex features.</div></div><span class="chip">AI</span></div><button class="btn primary" id="open-support">Open support chat</button></div>';
}
...
async function saveTheme(){
 const theme=$("#theme-choice")?.value||S.p?.theme||"dark",accent=$("#accent-choice")?.value||S.p?.accent_color||"#a98cff";
 if(window.VertexTheme)window.VertexTheme.apply(theme,accent);
 const x=await db.rpc("vertex_update_own_preferences",{p_theme:theme,p_accent_color:accent});
 if(x.error){toast(x.error.message||"Could not save appearance settings.",false);return}
 if(S.p){S.p.theme=theme;S.p.accent_color=accent}
 const v=$("#accent-value");if(v)v.textContent=accent;
